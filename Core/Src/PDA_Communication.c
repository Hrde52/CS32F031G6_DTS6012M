...
void HandleHeartbeat(SensorProtocol *pkt) {
    ...
    resp_data[offset++] = 30;
		if(PARA_TABLE_USE.data.functionChioce == 0x33)
		{
			WriteU16LittleEndian(&resp_data[offset], PARA_TABLE_USE.data.dts6012StudyDistance);
		}

    offset += 2;
    WriteU16LittleEndian(&resp_data[offset], /*detectTime*/PARA_TABLE_USE.data.dts6012DistanceChkThreshold);
    offset += 2;
...
void HandleControl(SensorProtocol *pkt) {
    ...
    uint8_t setNd06En_PDA = m_data3[3];    // 2
		uint8_t setResetParaFlg = m_data3[4];
    // byte4-5
        ...
        case 0xAA:
            ...
            // sensor_status = 2; 
						...
						if ((PARA_TABLE_USE.data.dts6012StudyDistance != dts6012DistancdLearnValue) 
							&& (dts6012DistancdLearnValue > PARA_TABLE_USE.data.dts6012MinChkDistance)) 
						{
    ...
    case 0XAA:
        if (DistanceThresholdLearningSuccessFlg == 1) {
            ...
            // 5-6  22  23
            WriteU16LittleEndian(&resp_data[22], 0); //   PARA_TABLE_USE.data.nd06StudyDistance
        } // if (DistanceThresholdLearningSuccessFlg == 1)
